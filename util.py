...
def quiz_submissions_list2dict(
    quiz_submissions_list: list[str],
) -> list[dict[str, any]]:
    return [json.loads(item) for item in quiz_submissions_list]


def quiz_submissions_dict2list(
    quiz_submissions_dict: list[dict[str, any]]
) -> list[str]:
    return [json.dumps(item) for item in quiz_submissions_dict]
...
def insert_database(insert_answer_function, quiz_item: dict):
    ...
    answer_content_list = get_answer_content_list(
        quiz_type, quiz_item["quiz"]["quizOptionses"], answer_id
    )
    answer_content = "\n".join(answer_content_list)  # 题目选项的内容

    print(
        f"quizId: {quiz_id}\t answerId: {answer_id}\t quizType:{quiz_type}\n"
